...
def get_number_classes(graph: rdflib.Graph):
    # fails
    ans = graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT DISTINCT ?s WHERE { ?s rdf:type owl:Class. FILTER isURI(?s) }')
...
def get_number_properties(graph: rdflib.Graph):
    # fails
    ans = graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT DISTINCT ?prop WHERE {{?prop a owl:DatatypeProperty} UNION {?prop a owl:ObjectProperty}}')
    return len(ans)

def get_number_instances(graph: rdflib.Graph):
    ans = graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT ?type WHERE { ?s rdf:type ?type. ?type rdf:type owl:Class }')
    return len(ans)
...
def get_number_properties_datatype(graph: rdflib.Graph):
    #fails
    ans = graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT DISTINCT ?property WHERE { ?property a owl:DatatypeProperty }')
    return len(ans)
...
def get_number_role_assertions(graph: rdflib.Graph):
    ...
    ra = len(graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT * WHERE { ?x ?w ?y. ?w a owl:ObjectProperty}'))
    ans = ra
    return ans
...
def get_number_GCI(graph: rdflib.Graph, mat=None):
    ...
    if mat==None or not mat:
        eq = 2 * len(graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT * WHERE { ?x owl:equivalentClass ?y }'))
        ans = sco + eq
...
def get_number_range_axioms(graph: rdflib.Graph):
    ...
    ra = len(graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT * WHERE { ?x rdfs:range ?y; a owl:ObjectProperty }'))
    ans = ra
    return ans
...
def get_number_role_inclusion(graph: rdflib.Graph, mat=None):
    ...
    cp = len(graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT DISTINCT ?x WHERE { ?x owl:PropertyChainAxiom ?y}'))
    fp = len(graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT ?x WHERE { ?x a owl:FunctionalDataProperty}'))
    ans = spo + tp +rp + cp + fp

    if mat==None or not mat:
        eq = 2 * len(graph.query('PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT * WHERE { ?x owl:equivalentProperty ?y }'))
        ans = ans + eq
